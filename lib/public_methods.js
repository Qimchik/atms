...
checkWeekDays = function(year){
	var days = 0;
	var d = new Date(year+"-01-01");
	var dEnd = new Date(year+"-12-31");
	do{
...
logPto = function(o) {
	...
	if (coefficient) {
		...
		if (hrms) {
			var daysPlus=hrms.hireDate.month >= 7 ? 1 : 0
			daysPlus += (date.year - hrms.hireDate.year) * hrms.daysPlus
		...
		//alert(date.year)
		var workingDays = checkWeekDays(date.year) - ptoAllowance // - ptoCount.count
	...
	PTOHours.insert({
	  ...
	  , projectId: o.projectId
	  , taskId: o.taskId
	  , hours: Tasks.findOne({ _id:o.taskId }).isOvertime? 0 : o.hours * coefficient
	  , date: o.date
	})
}
...
getWeekDates = function() {
	...
	function getTotal(date) {
		...
		LoggedTime.find({userId: Meteor.userId()}).forEach(function (time) {
			var timeDate = getUTCDate(new Date(time.date.year, time.date.month, time.date.date));
			if (+timeDate === +date) {
				total += time.sold ? 0: parseFloat(time.hours);
			}
		});
		if (!total) {
			total = '';
...
getTasks = function(project, userId, currentWeek, locker, loggedTime) {
		...
		Tasks.find({projectId: project._id}).forEach(function (task) {
			...
			thisTasks.hours.forEach(function (hours) {
				if (hours.hours !== '') {
					thisTasks.total += task.title == 'PTO' ? 
					hours.date.getDay()==0 ? 0 : parseFloat(hours.hours) : parseFloat(hours.hours);
				}
			});
			...
			loggedTime.forEach(function (time) {
				if (time.userId === userId && time.taskId === task._id) {
					...
					hours.push({
						...
						date: timeDate,
						hours: time.hours,
						sold: task.title == 'PTO'?timeDate.getDay() == 0 ? true : false : false
					});
				}
			});
...
getPTO = function(userId, date) {
	...
	if (PTO.findOne({userId: userId}) && PTOHours.findOne({ptoId: PTO.findOne({userId: userId})._id})) {
		...
		pto.taken = +ptoTaken;
		pto.sold = +ptoSold;
		pto.endOfCurrent = pto.endOfPrior + pto.duringCurrent - pto.taken - pto.sold;
